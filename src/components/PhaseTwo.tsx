...
const PhaseTwo = () => {
  ...
  const processVotingResults = async (categoryId: string) => {
    ...
    // Count votes
    Object.values(categoryVotes).forEach(optionId => {
  ...
  const generateAIMessage = async (prompt: { role: string, content: string }) => {
    try {
      // Instead of showing "This is a simulated AI response..."
      // Generate more realistic responses based on the prompt context
      const promptLower = prompt.content.toLowerCase();
      
      // For introductions
      if (promptLower.includes('introduction') || promptLower.includes('give a brief introduction')) {
        const name = promptLower.match(/your name is (\w+)/i)?.[1] || '';
        const age = promptLower.match(/you are (\d+) years old/i)?.[1] || '';
        const occupation = promptLower.match(/work as a ([^.]+)/i)?.[1] || '';
        
        return `Hello everyone. I'm ${name}, ${age} years old, working as ${occupation}. I'm looking forward to our discussion on how we can improve education for refugee children.`;
      }
      
      // For policy preferences
      if (promptLower.includes('policy area') && promptLower.includes('prefer')) {
        const policyName = promptLower.match(/for the ([^.]+) policy area/i)?.[1] || '';
        const policyOption = promptLower.match(/prefer "([^"]+)"/i)?.[1] || '';
        
        return `I believe "${policyOption}" is the best approach for ${policyName}. Based on my experience, this approach would provide the most sustainable benefit for refugee students while respecting their unique needs.`;
      }
      
      // Default response
      return `I think we need to carefully consider the long-term impacts of these policies on both refugee children and our education system as a whole.`;
    } catch (error) {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {/* Stakeholder profiles - simplified version hiding detailed information */}
        <div className="md:col-span-1 space-y-4">
          ...
          {gameState.aiProfiles.map((ai) => (
            ...
            >
              ...
              {/* Hide detailed information about AI participants */}
              {ai.speaking && (
        ...
        {/* Discussion area - improved colors */}
        <div className="md:col-span-3 flex flex-col h-[600px] border border-gray-300 rounded-lg shadow-md">
          {/* Messages display */}
                  ...
                  {gameState.policyCategories
                    ...
                    ?.options.map((option) => (
                      <button
                        ...
                        className="w-full text-left p-3 border border-gray-200 rounded-md hover:bg-indigo-50 hover:border-indigo-300 transition-colors"
                      >
                        <div className="font-medium text-indigo-700">{option.title}</div>
                        <div className="text-sm text-gray-600 mt-1">{option.description}</div>
                        <div className="text-xs text-amber-700 mt-2 font-medium">Cost: {option.cost} units</div>
                      </button>
                    ))
                  }
                </div>
          ...
          {/* Input area - improved colors */}
          <div className="p-4 border-t border-gray-300 bg-white rounded-b-lg">
            <div className="flex items-center">
              <button
                onClick={toggleSpeechRecognition}
                className={`p-2 rounded-full mr-2 ${isRecording ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-700'} hover:opacity-90 transition-opacity`}
      ...
      {/* Progress tracking - improved colors */}
      <div className="mt-6 bg-white p-4 border border-gray-300 rounded-lg shadow-sm">
